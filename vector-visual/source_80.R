...
# http://fawn.unibw-hamburg.de/cgi-bin/Rwiki.pl?QuiverPlot
 ...
 positionCourseGraph <- function(x, y, course, scale=1, length=0.05)
{
    u = cos(course)
    v = sin(course)
    ...
    matplot(x, y, type="pl",cex=0)
    arrows(x, y, x + u, y + v, length = length*min(par.uin()))
}
 ...
 pos <- read.table("positions.txt", header=T,sep=",")
 positionCourseGraph(pos$x, pos$y, pos$course, 2, 0.4)
